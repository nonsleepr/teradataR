sum.td.data.frame <- function(tdf, ...) {
    ...
    for (i in 1:length(names(tdf))) {
        if (as.character(i) %in% names(exprs)) 
            query <- gettextf("SELECT SUM(%s) FROM %s %s", .td.gencolumnexpr(tdf[i]), obj, wc) else query <- gettextf("SELECT SUM(\"%s\") FROM %s %s", names(tdf)[i], obj, wc)
