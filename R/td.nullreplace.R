td.nullreplace <- function(tdf, col, oTable, oDatabase = "", rType = "literal", rVal = 0) {
    ...
    if (rType == "literal") 
        query <- gettextf("CREATE TABLE %s AS (SELECT %s FROM %s %s) WITH DATA", oObj, nms, obj, wc) else if (rType == "mean") 
        query <- gettextf("CREATE TABLE %s AS (SELECT %s FROM %s, (%s) A %s) WITH DATA", oObj, nms, obj, gettextf("SELECT %s FROM %s %s", 
            paste(.td.genstats(col, c("mean")), collapse = ","), obj, wc), wc) else if (rType == "mode") 
        query <- gettextf("CREATE TABLE %s AS (SELECT %s FROM %s, (%s) A %s) WITH DATA", oObj, nms, obj, .td.genmode(tdf, 
            col), wc) else if (rType == "median") 
        query <- gettextf("CREATE TABLE %s AS (SELECT %s FROM %s, (%s) A %s) WITH DATA", oObj, nms, obj, .td.genmedian(tdf, 
            col), wc) else query <- gettextf("CREATE TABLE %s AS (SELECT %s FROM %s %s) WITH DATA", oObj, nms, obj, wc)
