td.values <- function(tdf, col, group.by = NULL) {
    ...
    if (!is.null(attr(tdf[nm], "expressions"))) 
        fnc <- .td.objectIsNumeric2 else fnc <- .td.objectIsNumeric
    
    if (fnc(attr(tdf, "database"), attr(tdf, "tableName"), col)) 
        vals <- paste(values[1:5], collapse = ",") else vals <- paste(values[c(1, 5, 6)], collapse = ",")
    ...
    } else query <- gettextf("SELECT '%s' as col,%s FROM %s %s", nm, vals, obj, wc)
