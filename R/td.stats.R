td.stats <- function(tdf, col, which = "all", type = "population", group.by = NULL) {
    ...
    if (is.null(group.by)) 
        query <- gettextf("SELECT '%s' as col,%s FROM %s %s", nm, paste(stats, collapse = ","), obj, wc) else {
        gb <- paste("\"", names(group.by), "\"", sep = "", collapse = ",")
        query <- gettextf("SELECT %s,'%s' as col,%s FROM %s %s GROUP BY %s ORDER BY %s", gb, nm, paste(stats, collapse = ","), 
            obj, wc, gb, gb)
    }
    df <- tdQuery(query)
    if (is.data.frame(df)) 
